...
export default function Projects({ tab }) {
  return (
    <section id="projects" className="body-font my-masonry-grid">
      <div className="mx-auto text-center my-masonry-grid_column w-screen md:w-[50%]">
            ...
            (project) =>
              tab === project.title && (
                <a
                  href={project.link}
                  key={project.image}
                  className="sm:w-1/2 hover:rounded-lg hover:border hover:border-[#212425] transition-all duration-500  bg-transparent"
                >
                  <div className="block relative m-5">
                    <img
                      alt="gallery"
                      className="absolute object-cover"
                      src={project.image}
                    />
                    <div className="p-[24px] relative z-10  border border-gray-800 bg-gray-900 opacity-0 hover:opacity-100">
