...
export default function Projects({ tab }) {
  return (
    <section className="text-center w-[50%] grid grid-cols-2">
      {projects.map(
        (project) =>
          tab === project.title && (
            <a
              href={project.link}
              key={project.image}
              className="sm:w-1/2 grid-cols-2 w-full hover:rounded-lg hover:border hover:border-[#212425] transition-all duration-500  bg-transparent"
            >
